...
func AddressHandler(c *gin.Context) {
    ...
    var field7, field8, field9 string
    ...
    query := `
        ...
        WHERE field3 = ?
    `
    err := database.DB.QueryRow(query, postalCode).Scan(&field7, &field8, &field9)
    if err != nil {
    ...
    log.Printf("Fetched fields: field7: %s, field8: %s, field9: %s", field7, field8, field9)

    var fullAddress string
    if field9 != "以下に掲載がない場合" {
		fullAddress = field7  + field8 +  field9
	} else {
        fullAddress = field7  + field8
